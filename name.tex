...
In this work, we mathematically investigate the computational power of self-attention to recognize formal languages.
Across different settings (finite or infinite precision, soft or hard attention), we show strong theoretical limitations of the computational abilities of self-attention.
Transformers cannot compute \textsc{Parity}, nor basic recursion, unless the number of layers or heads increases with input size.
These results contrast with LSTMs, which can handle such languages.
...
Among regular languages, transformers can only robustly recognize counter-free languages.
...
Similarly, the finiteness of precision does not preclude recurrent neural networks  from behaving as if they could encode unbounded numerical information for all practical purposes (CITE).
We therefore theoretically investigate both finite and infinite precision transformers.
...
Furthermore, there is a choice between modelling soft attention and hard attention.
...
Again, we will examine both of these settings.
...
\cite{dehghani2018universal} argued that transformers are restricted computationally, motivating their proposal of a variant with adaptive and unbounded number of layers.
They argued that transformers cannot compute functions that require sequential processing of input, without providing futher details or proofs.
...
\section{Three Formal Languages} % : \textsc{Parity}

We will analyze the ability of transformers to recognize three formal languages, representing basic regular and context-free languages.
%\textsc{Parity}.
...
\paragraph{\textsc{Dyck}} is the language of correctly bracketed words consisting of `(' and `)'.
...
As \textsc{Dyck} is a deterministic counter language, finite-precision LSTMs are capable of perfectly modeling \textsc{Dyck}; though they perform less well for versions with multiple types of parentheses \cite{sennhauser2018evaluating}, which are not counter languages any more.
...
\paragraph{\textsc{Logical Formulas}} is the set of Boolean formulas evaluating to True. Formally, we consider formulas using $\wedge, \vee, \neg$ and atomic \textsc{true}.
...
The basic idea behind the proof is that, through setting a small fraction of the input symbols in a particular way, we can `capture the attention' of the transformer in such a way that it ends up ignoring almost all remaining input symbols.
...
Compared to that literature, the method we use to prove existence of suitable restrictions is novel, as these proofs from the Boolean circuit literature do not seem to carry over to self-attention with infinite precision. %The proofs have in common the use of the probabilistic method, but the proof method in \cite{furst1984parity} does not appear to generalize to self-attention with infinite precision.
...
\begin{corollary}
Transformers with hard attention cannot compute \textsc{Parity}, \textsc{Dyck}, or \textsc{BooleanFormula}.
\end{corollary}
\begin{proof}
Take $C=0.1$.
...
But flipping a single input bit changes the value, so the transformer's output cannot match membership in \textsc{Parity}.
...
\end{remark}
With this technical notion, we show that we can reduce layers, iteratively removing the lowest layer:
...
There is only one case in which we cannot find such a subsequence, which is if all $j_1, ..., j_n$ together depend only on $< ck$ inputs.
...
We will construct input restrictions using probabilistic arguments over i.i.d. random restrictions, and which will require a large amount of independence between the activations of different heads for this approach to succeed.
We therefore first need to ensure that the number of neighbors of each head is bounded.
...
Assume the number of such inputs is larger than $0.9n$ for infinitely many values of $n$.
Then, the number of pairs of inputs and depending level-2 heads would be at least $0.9 n \cdot 2 H k c > Hckn$ for $n$ sufficiently large. On the other hand, there are only $Hckn$ many pairs of inputs and depending level-2 heads.
Thus, the number of such inputs is at most $<0.9n$ (for all but finitely many $n$).
...
After this manipulation, every input has at most $2kcH$ many $k$-depending layer-2 heads.\footnote{For finitely many values of $n$ -- but by making $k$ larger after this manipulation, this will hold for all $n$.}


Furthermore, assume the number of inputs with $> 2cH$ depending layer-1 heads is $\geq 0.9n$.
Then the number of input to layer-1 connections is $>0.9n \cdot 2cH$ for $n$ large enough.
...
After we previous part, we are in the setting where every input has $\leq 2kcH$ many depending layer-2 heads, and consequently every layer-2 head has at most $f \leq 2c^2k^2H$ $k$-neighbors (for any $k$).
(Note that the transformations carried out above could at most have increased the value of $H$).
Also, has $\leq 2cH$ depending layer-1 heads.
...
The probability of $X_i$ decays exponentially in $k$ (where $q = 1-p$):
\begin{equation}
P(X_i) \leq (1-(q/2)^c)^{\frac{k}{2cH}} %(1-q/2)^{k} (1-(q/2)^{c-1})
\end{equation}
%TODO unclear
%\begin{proof}
...
We have $P(Y_i^s) = 1-(q/2)^c < 0$.
There is a set of $\geq \frac{k}{2cH}$ independent events among these TODO SHOW THIS.
Call these $Y_i^{t_1}, ..., Y_i^{\frac{k}{2cH}}$.
Then $X_i \subseteq \bigcap_{s=1}^{\frac{k}{2cH}} Y_i^{t_s}$, and thus $P(X_i) \leq \prod_{s=1}^{\frac{k}{2cH}} Y_i^{t_s} = (1-(q/2)^c)^{\frac{k}{2cH}}$.
...
\begin{proposition}
If we exchange a single input bit $x_i$ with the bit $x_i'$, then the difference between the resulting activations at the decoder layers is bounded as $O(\frac{1}{n})$.
\end{proposition}
