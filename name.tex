...
Any $Y_i^s$ can be statistically dependent on at most $c \cdot \frac{1}{\eta}cH/C = \frac{1}{\eta}c^2H/C$ other events $Y_i^{s'}$.
Therefore, there is a set of $\geq \frac{k}{\frac{1}{\eta}c^2H/C}$ independent events among these.
Call these $Y_i^{t_1}, ..., Y_i^{\frac{k}{\frac{1}{\eta}c^2H/C}}$.
Then $X_i \subseteq \bigcap_{s=1}^{\frac{k}{\frac{1}{\eta}c^2H/C}} Y_i^{t_s}$, and thus
\begin{equation}
    P(X_i) \leq \prod_{s=1}^{\frac{k}{\frac{1}{\eta}c^2H/C}} P(Y_i^{t_s}) = \left(1-(q/2)^c\right)^{\frac{k}{\frac{1}{\eta}c^2H/C}}
\end{equation}
...
We want to show that we can find $q \in (0,1)$, $A, B \in (0,1)$ such that the following holds, assuming $f \leq \frac{1}{\eta}c^2k^2H/C$:
\begin{align}
 P(X_i) &\leq  D^k \leq A(1-B)(1-A)^f \\
P(X_0) &\leq \exp\left(-\frac{\delta^2qn}{(2+\delta)}\right)  \leq B (1-A)^{Hn}
\end{align}
where $D$ is a constant in $(0,1)$.
Once we have this, then by the asymmetric Lov{\'a}sz Local Lemma \cite{mitzenmacherprobability}, there is some input restriction that avoids all events $X_0, X_1, X_2, \dots$.

Choose %$q=0.5$, $\delta=0.5$, 
$A=\frac{1}{\frac{1}{\eta}k^2c^2/C}$, $B=0.5$.
...
For the RHS, 
\begin{equation}
\lim_{k\rightarrow \infty} A^{1/k} = \lim_{k\rightarrow \infty} \exp\left(-\frac{\log\left(\frac{1}{\eta} k^2 c^2/C\right)}{k}\right) = 1
\end{equation}
Also, $\lim_{k\rightarrow \infty} (1-A)^{\frac{1}{\eta}kc^2H/C}$ equals
\begin{equation}
\lim_{k\rightarrow \infty} \left(1-\frac{1}{Ek^2}\right)^{Ek} = \lim_{k\rightarrow \infty} \left(1-\frac{\sqrt{E}}{k^2}\right)^{k} = 1
\end{equation}
where we choose $E = \frac{1}{\eta}c^2H/C$. So, if we choose $k$ large enough, the RHS can be made arbitrarily close to $1$, in particular, greater than the LHS.
...
In conclusion, there exists a restriction $\rho'$ that avoids all events $X_0, X_1, X_2, ...$.
...
We now note that, since every layer-1 head depends only on $\leq ck$ many inputs after applying $\rho'$, each $y_j^{(1)}$ only depends on $\leq Hck$ many inputs.
We can thus remove layer 1, and obtain a $(Hck)$-transformer performing the same computation when $\rho'$ is applied.
