...
\begin{abstract}
    ...
    Transformers cannot do PARITY, nor basic recursion, unless the number of layers or heads increases with input size.
...
We consider this illustrative example:

\textsc{Parity} is the set of bit strings such that the number of $1$s is even.
Why is \textsc{Parity} interesting?
%Besides the general relevance of parity to computer science, there are specific reasons that make this relevant to NLP:
The reason is that Parity is a very simple instantiation of certain properties that make a large range of problems hard for self-attention.
If transformers cannot compute parity, it immediately follows that they also cannot compute various more complex functions:

\begin{enumerate}
\item \emph{Evaluating Logical Formulas:} A system that evaluates logical formulas (CITE) has to (among other things) be able to handle iterated negations.
...
\item \emph{Evaluating Recursion:} Recognizing whether an expression is bracketed correctly is a basic example of a context-free language and of recursion. If transformers cannot compute parity, they also cannot decide whether long expressions are bracketed correctly. See REF for proof.

\item \emph{Finite-State Languages with Groups:} The regular languages form the lowest layer of the Chomsky hierarchy, and even simple RNNs can compute all regular languages.
Parity is a regular language; moreover, all regular languages whose syntactic monoid contains a group are at least computationally as hard as Parity.
This means, if transformers cannot compute parity, their ability to emulate finite automata is strongly restricted, compared even to simple RNNs.
\end{enumerate}
...
Here we define self-attention following Vaswani et al.

We have an input $x_1,...,x_n$.

Sequence $p_1, p_2, ...$ of positional embeddings.

Layers:

(1) Multi-head self-attention

(2) position-wise fuly connected (brings together the heads)
...
This approach is inspired by a celebrated proof in the theory of Boolean circuits~\cite{furst1984parity}, but the method we use to prove existence of suitable restrictions is novel (The proofs have in common the use of the probabilistic method, but the proof method in \cite{furst1984parity} appears to be not powerful enough to generalize to self-attention with infinite precision).
...
\begin{equation}
|\{i \leq n: \rho_n(i) = *\}| \geq \frac{n}{10}
\end{equation}
and each second-layer head only depends on $c'$ inputs, for some constant $c'$ independent of $n$.
\end{lemma}
...
Let $H$ be the number of heads in layer 2.
...
This head depends on $\leq ck$ many inputs; by the Pigeonhole principle, this means, there is some input that has $> 2kcH$ many $k$-depending layer-2 heads.
We now show that the number of such inputs is small:
...
After we previous part, we are in the setting where every 2-layer head has at most $f \leq 2c^2k^2H$ $k$-neighbors (for any $k$).
...
\exp\left(-\frac{\delta^2(1-p)}{(2+\delta)}\right)  &\leq B^{1/n} (1-A)^H
\end{align}
