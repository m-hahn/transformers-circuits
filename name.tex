...
Why is Parity relevant?

\begin{enumerate}
\item For logical formulas, have to at least do this

\item Also, all regular languages whose syntactic monoid contains group are at least as hard as Parity.

\item Majority, and then also $a^nb^n$ with neutral letter.
\end{enumerate}
...
\begin{enumerate}
\item Case A: The number of inputs without $> 2kc$ many $k$-depending layer-2-heads is $\Omega(n)$ as $n\rightarrow \infty$, in which case we can just fix these bad inputs (whose complement is asymptotically at least linearly large) to a fixed value. After this manipulation, $f \leq 2k^2c^2$, and the number of heads might now be a constant multiple of $n$.

\item Case B: the number of inputs without $> 2kc$ many $k$-depending layer-2-heads is $o(n)$ as $n\rightarrow\infty$, so the number of inputs with $> 2kc$ attached layer-2-heads dominates any linear fraction. This is a contradiction, as this would entail $>ckn$ many such $k$-of-interest  connections between inputs and level-2 heads.
\end{enumerate}
