...
These results contrast with LSTMs, which can model such languages perfecty.
...
For \textsc{2Dyck}, we take a slightly different approach, taking the uniform distribution of well-bracketed strings of length $n$, and predicting the set of valid next characters, which is a subset of $\{(,),[, ]\}$.
...
\begin{corollary}
Transformers with hard attention cannot compute \textsc{Parity}, \textsc{Dyck}, or \textsc{BooleanFormula} (i.e., asymptotic cross-entropy is at chance level).
\end{corollary}
\begin{proof}
Take $C=0.1$.
...
In the previous section, we showed that transformers using hard attention are not able to recognize a ange of core formal languages.
