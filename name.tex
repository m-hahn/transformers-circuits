...
First, assume there is some input that has $>2kcH/C$ many $k$-depending layer-1 heads.
...
After this manipulation, every input has at most $2kcH/C$ many $k$-depending layer-1 heads, independent of $n$. 

Furthermore, assume the number of inputs with $> 2cH/C$ depending layer-0 heads is $\geq 0.9Cn$.
Then the number of pairs of inputs and layer-0 heads is $>0.9Cn \cdot 2cH/C > ncH$.
But there are at most $ncH$ such pairs, contradiction.
So the number of inputs with $> 2cH/C$ depending layer-0 heads is $\leq 0.9Cn$.
We can again restrict these inputs to some fixed value (again, it doesn't matter which one).

After these preparations, 
\begin{equation}
|\{i \leq n: \rho_n(i) = *\}| \geq \frac{C}{100}n
\end{equation}
...
After the previous part, we are in the setting where every input has $\leq 2kcH/C$ many depending layer-1 heads, and consequently every layer-1 head has at most $f \leq 2c^2k^2H/C$ $k$-neighbors (for any $k$).
%(Note that the transformations carried out above could at most have increased the value of $H$).
Also, every input has $\leq 2cH/C$ depending layer-0 heads.
...
For each input length $n$, define the distribution that independently assigns to each input position $i \in \{1, \dots, n\}$ the symbol $*$ with probability $p=0.5$, and $1$ or $0$ with equal probability else.
...
\end{equation}
we choose $q, \delta$ as desired, and then make $n$, $k$ large enough to satisfy the inequality. %so large that $B^{1/n} > 0.99$ and $(1-A)^H = (1-\frac{1}{2k^2c^2})^H > 0.99$.


With such a $\rho'$, we have
\begin{equation}
|\{i \leq n: \rho_n'(i) = *\}| \geq \frac{C}{100}\cdot (3/4)\cdot pn
\end{equation}
