...
\title{Theoretical Limitations of Self-Attention}
...
We show that neither of these problems can be solved by transformers and similar models relying entirely on self-attention.
...
Our results therefore also yield more generally strong limitations on the ability of self-attention to model finite-state languages except for the counter-free ones, and to model wider classes of context-free languages.
...
Our study provide the first theoretical results about limitations in the power of transformers.
...
While we consider transducing inputs to a single label (0, 1), they study the setting in which the transformer computes an unbounded number of autoregressive decoding steps, not bounded in the input length $n$.
...
In practice, soft attention is easier to train with gradient descent methods; however, analysis studies suggest that attention often concentrates on one or a few positions \cite{voita2019analyzing,clark2019bert} and that the most important heads tend to be those that clearly focus on a few positions~\cite{voita2019analyzing}, suggesting that attention often behaves  like hard attention in practice. %, and analysis of neural network attention focuses on the positions where attention concentrates
...
Consequently, the ability of LSTMs to model \textsc{2Dyck} has been an object of experimental study \cite{sennhauser2018evaluating,skachkova2018closing,bernardy2018can}.
...
%In contrast, 
Our theoretical results will show that transformers are strongly limited in their ability to model \textsc{2Dyck}, or variants with fewer or more types of parentheses.
...
$1^*$ (over the alphabet $\{0,1\}$) is the regular language of words that have only ones and no zeroes; its minimal automaton has two states, like \textsc{Parity}.
...
In general, the depth reduction method applies to languages that are sufficiently \emph{sensitive}: If fixing $\Theta(n)$ input symbols does not force a word to be inside or outside of the language, and if every word of length $n$ in the language has $\Theta(n)$ many Hamming neighbors that are outside of the language, then self-attention cannot recognize this language.
Sensitivity of functions and the structure of Hamming neighborhoods have been studied in computational complexity~\cite{boppana1997average,gilmer2015new,gopalan2016smooth,rossman2018average} and more recently linked to generalization in feedforward networks~\cite{de2018deep}.
...
Then, after applying the resulting restriction, $y_{n}^{(L)}$ is determined by a bounded number of input bits.
\end{proof}
...
For each layer-1 attention head, we then select a sequence $1 \leq i_1 < i_2 < \dots < i_k \leq n$ such that (1) for each $i_s$, there is at least one input that only feeds into the element $j_{i_s}$ and no other $j_{i_s'}$, (2) $i_k$ is minimal, i.e. there is no subsequence with smaller $i_k$ that also satisfies (1).
...
We have $P(Y_i^s) = 1-(q/2)^c \in (0,1)$. % (where $q = 1-p$).
...
\end{align}
where $D =  \left(1-(q/2)^c\right)^{\frac{1}{\frac{1}{\eta}c^2/C}}$. % is a constant in $(0,1)$, depending on $c$, $q$, $C$, $\eta$, but \emph{not} on $k$.
...
In conclusion, there exists, for each $n$, a restriction $\rho'_n$ that avoids all events $X_0, X_1, X_2, \dots$.
...
We have shown that, even with infinite precision, transformers cannot robustly model periodic regular languages or basic hierarchical structure.
...
This clear contrast between infinite-precision LSTMs and our findings for infinite-precision transformers provides a theoretical explanation for the empirical finding that LSTMs seem to outperform transformers in modeling hierarchical structure \citep{tran2018importance}.
...
Our results entail that self-attention is strongly limited in their ability to model context-free languages or Boolean formula evaluation.
How should we reconcile this with the success of transformers at many natural language processing tasks?
One possibility is that strong quantitative performance at NLP tasks can be achieved without genuine understanding of linguistic structure, as has been argued for other neural network models used in NLP \cite{linzen2016assessing,marvin2018targeted}.
...
Intriguingly, self-attention bears some resemblance to models of memory in human sentence processing that assume that humans, while processing a word, attend to chunks that were stored in memory when processing some previous words \cite{lewis2005activation}.
Such models predict difficulty with center embedding because they cannot count brackets, akin to what we have shown theoretically for self-attention \cite{lewis2005activation}.
