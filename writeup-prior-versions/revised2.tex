...
We show that hard attention transformers cannot represent \textsc{Parity} or \textsc{2Dyck}, assuming a technical condition on attention distributions. %, or \textsc{BooleanFormula}.
To otherwise keep the results maximally general, our analysis will use combinatorial arguments and make no assumption about, e.g., activation functions and the norms of parameter matrices.
...
\paragraph{Technical condition:} There is a constant $F$ and a function $f(k) : \mathbb{N} \rightarrow \mathbb{N}$ (but independent of $n$) such that, for $s > F$, $|i - j_s^{(z)}| \leq f(k)$.
The condition requires that, while a head may place the highest attention to some position very far away (a head might plausibly want to attend to the beginning or end of a string). But the problem with this condition is that it rules out that a head wants to look for an occurrence of some specific input symbol combination. 
