...
\title{Theoretical Limitations of Self-Attention in Neural Sequence Models, VERSION3 with alternative soft attention proof}
...
While they focused on experiments with several NLP tasks, they also provide a theoretical analysis, showing that a single self-attention layer with a single head will be robust against input perturbations, assuming that input embeddings are drawn uniformly from the unit sphere.
One of our results, Theorem~\ref{thm:softmax-main-adapt}, can be seen as considerably widening the scope of their result, both by avoiding distributional assumptions, and by applying to transformers with arbitrary numbers of heads and layers.
...
We will use the smoothness of the operations used in transformers to show that any transformer, as inputs get longer, will not be able to robustly separate members of the languages from other inputs.
The proof will be slightly more general than softmax attention, by also covering models that have both hard and soft attention heads, as proposed by \citet{shen2018reinforced}.

%, and models that allow the attention logits $a_{i,j}^{(k,h)}$ to be infinite.
...
We say a function $R(x_{1,\dots,n})$ mapping to values in $\mathbb{R}^d$ is \key{$\epsilon$-stable for $x_j$} if flipping the value of $x_j$ and keeping the other inputs fixed can only change $R(x_{1,\dots,n})$ by a difference of norm $\leq \epsilon$.


\begin{thm}\label{thm:softmax-main-adapt}
Let any transformer be given, and let $C \in (0,1)$, $\epsilon > 0$.
Then there is a restriction $\rho$ and a constant $c \in \mathbb{N}$ % sequence of integers $c_n = o(n)$
such that 
$$|\{i \leq n: \rho_n(i) = *\}| \geq Cn$$
(for all sufficiently large $n$) and such that, for each $i$, there is a set of $\leq c$ inputs such that $y_i^{(L)}$ is $\epsilon$-stable for any other $\rho$-free individual input. %flipping any other individual input would change  by at most $\leq \epsilon$.
\end{thm}

We first show how this entails that transformers cannot correctly model the two formal languages:
\begin{corollary}
No continuous classifier built from $y_n^{(L)}$ can have perfect cross-entropy on classifying strings as belonging to \textsc{Parity}, or classifying as belonging to \textsc{2Dyck}.
\end{corollary}
\begin{proof}
The proof is as before, TODO
\end{proof}



\begin{defin}[$c$-Transformer]
Let $c$ a positive integer. A $c$-transformer of depth $d$ is one in which the layer-0 activations $y_j^0$ depend on the embeddings not just at one position $j$, but are a function of the embeddings at $\leq c$ input positions, up to a remainder:
\begin{equation}
    y_j^0 = f^{inp}_{n,j}((x_{i_1^{j,n}}, p_{i_1^{j,n}}), \dots, (x_{i_c^{j},n}, p_{i_c^{j,n}} )) + R_j(x_{1,\dots,n})
\end{equation}
where $R_j$ is $\epsilon$-stable for each other input. %with $\|R\| \leq \epsilon$.
Here, the indices $x_{i_s^{j,n}}$ depend on the input length $n$.
\end{defin}

In the soft attention setting, the Depth Reduction Lemma takes the following form:
\begin{lemma}[Depth Reduction Lemma]
Given a $(c,\epsilon)$-transformer with $d$ layers, and some restriction $\rho$ such that
\begin{equation}
|\{i \leq n: \rho_n(i) = *\}| \geq Cn
\end{equation}
($C \in (0,1]$)
...
\end{equation}
for all sufficiently large $n$, 
and such that the resulting function -- when applying $\rho'$ -- is computed by a $(H\cdot c\cdot k, \epsilon')$-transformer with $d-1$ layers, where $H \geq 1$ is the number of attention heads at each layer and position, for some integer $k \in \mathbb{N}$, and for some $\epsilon' = O(\epsilon)$ (with constants depending on the parameters of the transformer, but independently of $n, k$).
\end{lemma}

The proof is almost identical to the hard attention setup.
Rather than repeating the entire proof, we will explain those points where it differs from the preceding proof.

The Preliminaries are \textit{verbatim} identical.
In the construction of restrictions, we make the following changes.
...
We keep $X_0$ as before.
Let $X_i$ be the event that, for the $i$-th head, no size $ceil(1/\epsilon)$ subset of $j_{i_1}, \dots, j_{i_k}$ are assigned the value that produces the highest attention score, up to difference $\delta$ (TODO)?????????


TODO cannot exactly set any layer-0 activation. can only get it up to $\delta$ of the real intended attention value.

Then $\Prob(X_i) \leq E_1 \exp(-E_2k)$ where the constants $E_1, E_2$ depend on $\eta, c, C, \epsilon$.

The proof that all events $X_0, X_1, \dots$ can be avoided by some restriction $\rho'_n$ is exactly as in the hard attention case.

%We now want to show that each layer-1 activation only depends on $\leq ckH$ inputs, up to a remainder of norm $\leq \epsilon$.

Now recall that every layer-1 activation $y_j^{(1)}$ is computed as
\begin{equation}
    y_i^{(1)} := f^{act}(y_i^{(0)}, b_{i,1,1}, \dots, b_{i,1,H})
\end{equation}
where 
\begin{equation}
    b_{i,1,h} = \sum_{j=1}^n \hat{a}_{i,j}^{(1,h)} y_j^{(0)}
\end{equation}


We know that for $j$ except for those in the size-$k$ relevant set, we have $\hat{a}_{i,j}^{(1,h)} \leq \epsilon$.
Thus
\begin{equation}
    b_{i,1,h} = \sum_{i_1, \dots i_k} \hat{a}_{i,j}^{(1,h)} y_j^{(0)} + \sum_{other j} O(\epsilon) y_j^{(0)}
\end{equation}
Every input except for those that $c$-feed into some of the $y_j^{(0)}$ ($j$ in the good set) will have impact $\leq \epsilon$ on any of these terms in the left sum.
If we choose $E$ such that $\|y_j^{(0)}\| \leq E$ (this will be a function of the parameter matrices of the transformer), then any input has impact $\leq \epsilon E$ on any summand on the right, and $\leq E$ on the right sum.
So it will also have impact $O(\epsilon)$ on the entire term (where the constants do not depend on $n, i, h, k$).

As a consequence every layer-1 activation $y_j^{(1)}$
\begin{equation}
    y_j^{(0)} = f^{inp}_{n,j}((x_{i_1^{j,n}}, p_{i_1^{j,n}}), \dots, (x_{i_c^{j},n}, p_{i_c^{j,n}} )) + R_j
\end{equation}
where  $R_j$ is $O(\epsilon)$-stable on any other input (where $\mathcal{O}$ covers constants independent of $n, j, k$).
This concludes the proof of the Soft Attention version of the Depth Reduction Lemma.
...
Our application of the Lov{\'a}sz Local Lemma leaves room for tightening, and it would be possible to bring the probability mass of the restrictions $\rho'$ avoiding $X_0, X_1, ...$  arbitrarily close to $1$ by choosing $k$ dependent on $n$, at the cost of a more complicated proof.
As a consequence, it would be possible to show that Theorem~\ref{thm:softmax-main-adapt} holds for `almost all' restrictions $\rho$, and to derive stronger gaps between the optimal cross-entropy and the cross-entropy achievable by transformers for problems such as \textsc{2Dyck} and \textsc{Parity}.
