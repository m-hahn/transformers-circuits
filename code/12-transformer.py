...
#####################################
# 10: Default + large batch size (500) can solve length 8, though not length 10.
# WORKS
# ./python36 12-transformer.py --sequence_length 8 --n_layers 3 --batchSize 1000
# FAILS
# ./python36 12-transformer.py --sequence_length 9 --n_layers 3 --batchSize 1000
# WORKS (found by bayesian search)
# ./python36 12-transformer.py --V 3 --beta1 0.95 --beta2 0.95 --warmup 2000 --batchSize 500 --epochCount 100 --n_layers 2 --d_model_global 64 --d_ff_global 1024 --h_global 2 --dropout_global 0.0 --sequence_length 9
# ./python36 12-transformer.py --V 3 --beta1 0.95 --beta2 0.95 --warmup 2000 --batchSize 500 --epochCount 100 --n_layers 2 --d_model_global 64 --d_ff_global 1024 --h_global 2 --dropout_global 0.0 --sequence_length 10
...
parser.add_argument("--d_model_global", dest="d_model_global", type=int, default=64)
...
def make_model(src_vocab, tgt_vocab, N=6, 
               d_model=d_model_global, d_ff=d_ff_global, h=h_global, dropout=dropout_global):
...
def data_gen(V, batch, nbatches):
    ...
    for i in range(nbatches):
        data = torch.from_numpy(np.random.randint(1, V, size=(batch, random.randint(2,sequence_length)))).cuda()
        data[:, 0] = 1
...
model = make_model(V, V, N=n_layers)
model_opt = NoamOpt(model.src_embed[0].d_model, factor, warmup,
        torch.optim.Adam(model.parameters(), lr=0, betas=(beta1, beta2), eps=eps))
