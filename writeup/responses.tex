...
\item \response{In the third and fourth paragraph, we explain what we see as the two main theoretical implications of our results:}
...
\item \response{Second, in the fourth paragraph, we discuss the implication that strong NLP methods can be based on models that are too weak for the language classes assumed in theoretical linguistics. This paragraph is largely taken over from the previous version, but we now also note the view of asymptotic results as formalizing generalization, mentioned by the editor (with references to Chomsky 1957 and Shieber 1985).}

%\response{We also considered discussing the relation between asymptotic properties and generalizable descriptions (as noted by the editor), but chose to not include this because transformers are currently used as NLP methods, not as the basis of grammatical description.}
\end{enumerate}
