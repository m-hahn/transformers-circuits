...
Note that, due to the construction of the attention weights $\hat{a}_{i,j}$, the activations $y_{i}^{(L)}$ only depend on input symbols $x_1, \dots, x_{i-1}$, so that $p_i(v)$ computes a distribution that is conditioned only on those preceding inputs: $p_i(x_i|x_1\dots x_{i-1})$.
...
This is related to the task of \key{language recognition}, the task of classifying input strings as either belonging to or not belonging to a formal language.
...
In this task, the transformer need not be autoregressive.
Here, for some formal language $L$ and a fixed probability distribution over strings, the cross-entropy is $- \mathbb{E}_{{\bf x}} \log p(\delta_{{\bf x} \in L}|{\bf x})$.
It is zero if and only if the model is both correct with perfect confidence on every input.
...
\begin{corollary}
Transformers with hard attention cannot model \textsc{Parity} or \textsc{2Dyck} with perfect cross-entropy, either in the prediction or recognition settings. %, or \textsc{BooleanFormula}.
...
Furthermore, no classifier built from $y_n^{(L)}$ can have perfect accuracy on classifying strings as belonging to \textsc{Parity}, or classifying as belonging to \textsc{2Dyck}.
\end{corollary}
\begin{proof}
Take $C=0.6$.
...
In this context, we note that our depth reduction method applies to languages that are sufficiently `sensitive' in the following  sense: 
\begin{corollary}
...
In the autoregressive setting, the change is $\mathcal{O}(\frac{1}{i})$ for $y_i^{(L)}$ for all $i$ except the one at which the change was made.
\end{lemma}
...
Then it cannot achieve perfect cross-entropy on either the prediction or recognition tasks.

Furthermore, in the prediction task, as $n\rightarrow\infty$, cross-entropy converges to chance level (1 bit per symbol).
\end{thm}
...
To reduce notational complexity, we only prove the lemma in the non-autoregressive case.
