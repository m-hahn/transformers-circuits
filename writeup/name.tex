...
then the change in the resulting activations $y_i^{(L)}$ is $\mathcal{O}(\frac{1}{i})$  for all $i$ except the one at which the change was made, with constants depending on the parameter matrices (independently of $i$ or the input length).
\end{lemma}
...
But PROBLEM: in the preparations, we already fix a constant fraction of the input.
