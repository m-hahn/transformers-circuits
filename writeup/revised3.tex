...
We first show that we can assume, without loss of generality, that each input bit has $\leq \frac{1}{\eta} c/C$ depending layer-0 heads.
...
We can restrict these input bits to some fixed value (it doesn't matter which one), and the set $\{i \leq n: \rho_n(i) = *\}$ still has at least $(1-\eta) C n$ elements.
...
We will construct input restrictions $\rho'_n$ using probabilistic arguments over randomly chosen restrictions.
...
Fix some $\eta \in (0,\frac{1}{2})$ (to be chosen later).
Let $H$ be the number of attention heads in each position of layer 1.




We now show that we can assume, without loss of generality, that each layer-0 head has at most $\leq 2^c kH$ many $k$-depending layer-1 heads.
Assume that indeed some layer-0 head has $>  2^c kH$ many $k$-depending layer-1 heads (combined with values $z$).
By fixing $\leq c$ input bits, we can fix this head to a value that achieves the maximum attention value for at least $> kH$ many of these $k$-depending layer-1 heads (due to the Pigeonhole Principle).
Once we have done this, 
$\{i \leq n: \rho_n(i) = *\}$ has at least $(1-\eta) C n - c$ elements, and $> kH$ many layer-1 heads (+value $z$) now already satisfy the Depth Reduction Lemma.
We then repeat the selection of the sequence $j_1^{(z)}, \dots, j_n^{(z)}$, and repeat the construction described here.
We iterate this procedure until no layer-0 head has $>  2^c kH$ many $k$-depending layer-1 heads.
...
Let $U$ the number of times this procedure is iterated ($U \leq \frac{2^c n}{k}$).
...
Given that $2\eta < \frac{1}{2}$, this is still a constant fraction of $n$.
Once this is completed, each layer-0 head has at most $\leq 2^c kH$ many $k$-depending layer-1 heads.
Thus each input bit has at most $\leq \frac{2^c}{\eta}kcH/C$ many $k$-depending layer-1 heads.
...
\end{equation*}
for all sufficiently large $n$.
Then choose $\eta \in (0,\frac{1}{2})$ small, $q \in (0,1)$ small, and $\delta >0$ (such that $(1+\delta)q \in (0,1)$) in such a way as to achieve $(1-2\eta)\cdot (1-(1+\delta)q) = C'/C$.
...
One can show that there is a constant $P_0 \in (0,1)$ (dependent on $p$ but not $n$), such that $x$ both ends with a closing bracket, and is unbalanced, with probability $\geq P_0$.\footnote{%If $n$ is odd, $x$ cannot be balanced.
