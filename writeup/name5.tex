...
We aim to prove that no hard-attention transformer is capable of representing \textsc{Parity} or \textsc{2Dyck}, by constructing -- for any given transformer model -- a set of input words that this model will have to misclassify.
...
This barrier prevents us from proving bounds on the \emph{accuracy} that soft attention transformers can achieve; nevertheless, we will be able to prove limitations on the achievable \emph{perplexity} in predicting the next symbol in an input.
...
We proceed to proving the technical Lemma~\ref{lem:soft-tech}:
...
\begin{align*}
\leq L_{f^{act}} \cdot \left((2C)^{2(k-1)}D + o(1)\right) \leq (2C)^{2k}D
\end{align*}
