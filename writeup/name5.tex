...
Our study provides the first theoretical results on limitations in the power of transformers.
...
We aim to prove that no transformer is capable of representing \textsc{Parity} or \textsc{Dyck}, by constructing -- for any given transformer -- a set of input words that this model will have to misclassify.
...
Our approach for proving Theorem~\ref{thm:hardmax-main} will be to iteratively remove the lowest layer of a given transformer by applying input restrictions, until only the final prediction $y_n^{(L)}$ remains. %by constructing input restrictions.
In order for this to go through, the main challenge is to construct suitable restrictions:
As illustrated in Figure~\ref{fig:restr}, these should only affect a few input bits (namely, $Cn$ many input bits), while forcing each attention head in the first layer to ignore all but $c$ input bits.
