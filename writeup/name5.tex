...
This barrier prevents us from proving bounds on the \emph{accuracy} that soft attention transformers can achieve; nevertheless, we will be able to prove limitations on the achievable \emph{cross-entropy} in predicting the next symbol in an input.
...
\begin{align*}
  &  L_{f^{act}} \cdot \frac{H^{k-1} C^{(k-1)}D + H^{k} C^{k} D}{n}
  %&   \\
\end{align*} 
