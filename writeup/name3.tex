...
The proof will be slightly more general than softmax attention, by also covering models that have both hard and soft attention heads.
...
such that 
$$|\{i \leq n: \rho_n(i) = *\}| \geq Cn$$
(for all sufficiently large $n$) and such that, for each $i$, there is a set of $\leq c$ inputs such that flipping any other individual input would change $y_i^{(L)}$ by at most $\leq \epsilon$.
\end{thm}
...
\end{equation}
where $R_j$ is $\epsilon$-stable. %with $\|R\| \leq \epsilon$.
Here, the indices $x_{i_s^{j,n}}$ depend on the input length $n$.
\end{defin}
...
\end{equation}
for all sufficiently large $n$, 
and such that the resulting function is computed by a $(H\cdot c\cdot k, \epsilon')$-transformer with $d-1$ layers, where $H \geq 1$ is the number of attention heads at each layer and position, for some integer $k \in \mathbb{N}$, and for some $\epsilon' = O(\epsilon)$ (with constants depending on the parameters of the transformer, but independently of $n, k$).
\end{lemma}
...
Let $X_i$ be the event that, for the $i$-th head, no size $ceil(1/\epsilon)$ subset of $j_{i_1}, \dots, j_{i_k}$ are assigned the value that produces the highest attention weight.
...
\begin{equation}
    y_j^{(0)} = f^{inp}_{n,j}((x_{i_1^{j,n}}, p_{i_1^{j,n}}), \dots, (x_{i_c^{j},n}, p_{i_c^{j,n}} )) + R
\end{equation}
where the dependence of $R$ on any input is $O(\epsilon)$ (with constants independent of $n, j, k$).
