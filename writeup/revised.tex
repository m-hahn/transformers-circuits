...
- if even: emit with probability $p/2$ each, terminate with prob $1-p$

- if odd: emit with probability $1/2$ each
...
Consider a string $x$ of length $n$.
...
For instance, consider a string $x$ where the next symbol can be `)' -- or an opening bracket -- but not `]'.
Then there is a string $x'$, differing only in one input position, where $p_{\textsc{2Dyck}}(x) = p_{\textsc{2Dyck}}(x')$, but for which the next symbol can be `]' but not `)'.
As before, a Lipschitz-continuous prediction function thus cannot robustly assign different next-symbol probabilities for `)' vs `]'.
...
A similar proof strategy can also be applied to other fixed-depth architectures that combine unboundedly many inputs in a smooth manner, such as 1D temporal convolutions with average pooling (TODO CITE).
...
Our results thus do not constitute conclusive evidence for limitations of large real-world systems.
...
\paragraph{Limitations in Human Capabilities}
...
\section{Conclusion}
We formally investigated the capabilities of self-attention in modeling regular languages and hierarchical structure, in the limit of long inputs.
