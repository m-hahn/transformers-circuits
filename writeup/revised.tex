...
Specifically, the implementation described by \citet{vaswani2017attention} linearly transforms the position-wise activations $y_i^{(k-1)}$ separately into `query vectors' $Q y_i^{(k-1)}$ and `key' vectors $K y_i^{(k-1)}$ (for some parameter matrices $K, Q$); %are transformed separately into query $Q^{(k-1,h)}(y_i^{(k-1)})$ and key $K^{(k-1,h)}(y_i^{(k-1)})$ vectors;
...
How long does an input have to be for the results to start becoming relevant?
...
However, it is unclear how far this bound is from optimality. Also, note that our proof for hard attention makes no assumption about activation functions, and stronger bounds might be obtained by restricting to realistic functions used in implementations.
...
On the other hand, it has been observed that neural networks may require far fewer parameters for computing a function than typically used, and that overparamaterization is mainly beneficial for the learning process, and not necessary for computational capabilities (CITE). TODO how far has BERT been compressed.
\citep{voita2019analyzing,michel2019sixteen}.
When pruning networks \citet{michel2019sixteen} show that some BERT layers can be reduced to just a single attention head.


Understanding this theoretically is an important and challenging problem for future research.
...
What is the value of such an
asymptotic property? (unexpressability in the
limit = ungeneralizability)?
